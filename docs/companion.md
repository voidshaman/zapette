...
TCP socket bound to the TV's **own loopback** on port 7900 (`dist/zapette-companion.apk`, built by
...
"First connect" is a file, not a guess: `~/.config/zapette/device-<device>.json`, keyed on the adb
...
`y` (or Enter) runs the setup as five steps, each with its own verdict, stopping at the first failure:
    ...
    ✓ preparing the APK — using zapette-companion.apk (45602 bytes, newer than the sources)
...
The APK step prefers the prebuilt `dist/zapette-companion.apk` when it is newer than the companion's sources
