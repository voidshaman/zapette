...
//
// The APK is checked into this tree's dist/ and rebuilt by companion/build.sh
// (aapt2 + javac + d8 + apksigner, no Gradle, no network), or by its Windows twin
// companion/build.cmd on win32.
...
/** The build script for this platform: the same steps, run by cmd.exe on Windows. */
export const BUILD_SCRIPT = process.platform === "win32" ? "companion/build.cmd" : "companion/build.sh"
const SOURCE_DIR = "companion"
// Intermediates of a --debug build; they are not inputs.
const SKIP = new Set(["build", ".DS_Store"])
// What the build scripts actually read: java, resources/manifest XML, and the
// script itself (either flavour). Notes (README.md, MEASUREMENTS.txt) are not
// inputs — editing one must not rebuild the APK, or a documentation pass would
// make every install rebuild.
const INPUT_EXTENSIONS = new Set(["java", "xml", "sh", "cmd"])
...
/**
 * Run the platform's build script — build.sh, or build.cmd through cmd.exe on
 * Windows. Never throws: a missing toolchain comes back as `ok:false` with the
 * script's own sentence ("no JDK found …") in `tail`, which is the thing worth
 * showing the user.
 */
export function buildCompanionApk({ root = repoRoot(), timeoutMs = 240000, onLine } = {}) {
  return new Promise((resolve) => {
    ...
    let child
    try {
      // bash runs the .sh. The .cmd needs a shell, and /d /s /c keeps it
      // non-interactive and predictable (no autorun, no echo of the command).
      child =
        process.platform === "win32"
          ? spawn("cmd.exe", ["/d", "/s", "/c", script], { cwd: root, stdio: ["ignore", "pipe", "pipe"], windowsHide: true })
          : spawn("bash", [script], { cwd: root, stdio: ["ignore", "pipe", "pipe"] })
    } catch (e) {
      return resolve({ ok: false, error: `could not start the build script: ${e?.message ?? e}`, ms: 0, tail: "" })
    }
    const lines = []
    const feed = (chunk) => {
